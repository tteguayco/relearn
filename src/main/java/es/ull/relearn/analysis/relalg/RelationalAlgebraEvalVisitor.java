...
public class RelationalAlgebraEvalVisitor extends RelationalAlgebraBaseVisitor<String> {
	...
	private String[] splitAttrList(RelationalAlgebraParser.ProjectionContext ctx) {
		String attrList = visit(ctx.attrlist().get(0));
	...
	private String preppendSelectStarIfNeeded(RelationalAlgebraParser.ExprContext ctx, String translation) {
		...
		// example: (PROJECT [A, B, C] (r1 x S1)) X (r2 / S2);
	}
	...
	@Override
	public String visitRename(RelationalAlgebraParser.RenameContext ctx) {
		String renameStatement = "SELECT ";
		
		// expr inside RENAME can only be a PROJECTION
		if (ctx.expr() instanceof RelationalAlgebraParser.ProjectionContext) {
			RelationalAlgebraParser.ProjectionContext expr = (RelationalAlgebraParser.ProjectionContext) ctx.expr();
			
			String newAttributesNames = visit(ctx.attrlist());
			String oldAttributesNames = visit(expr.attrlist().get(0));
			String translatedExprFromProjection = visit(expr.expr());
			
			String[] newAttributesNamesSplitted = newAttributesNames.split(",\\s+");
			String[] oldAttributesNamesSplitted = oldAttributesNames.split(",\\s+");
			
			if (newAttributesNamesSplitted.length == oldAttributesNamesSplitted.length) {
				for (int i = 0; i < newAttributesNamesSplitted.length; i++) {
					String auxNewAttr = newAttributesNamesSplitted[i];
					String auxOldAttr = oldAttributesNamesSplitted[i];
					
					renameStatement += auxOldAttr + " AS " + auxNewAttr;
					
					// Add comma if needed
					if (i < newAttributesNamesSplitted.length - 1) {
						renameStatement += ", ";
					}
				}
				
				renameStatement += " FROM " + translatedExprFromProjection;
			}
			
			else {
				appendError("The length of the attributes list of the RENAME statement does not match " 
							+ "the length of the attributes list of its inner PROJECTION");
			}
		}
		
		else {
			appendError("A RENAME statement should contain an inner PROJECTION");
		}
	...
	@Override 
	public String visitNotEqual(RelationalAlgebraParser.NotEqualContext ctx) {
		return "<>";
	}
